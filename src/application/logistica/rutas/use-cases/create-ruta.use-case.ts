...
export class CreateRutaUseCase {
  ...
  async execute(createRutaDto: CreateRutaDto) {
    try {
      ...
      const fechaFin = createRutaDto.fecha_fin
        ? new Date(createRutaDto.fecha_fin)
        : null;
      ...
      if (createRutaDto.id_conductor) {
        const conductorDisponible =
          await this.rutaRepository.isConductorDisponible(
      ...
      if (createRutaDto.id_vehiculo) {
        const vehiculoDisponible =
          await this.rutaRepository.isVehiculoDisponible(
